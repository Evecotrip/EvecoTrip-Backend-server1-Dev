...
// ============================================
// src/middleware/auth.middleware.ts - SUPABASE AUTH VERSION
// ============================================
import { Request, Response, NextFunction } from 'express';
import { verifyToken } from '../utils/jwt.utils';
import { RoleType } from '@prisma/client'; 
...
export const authenticateJWT = async (
  ...
  try {
    ...
    if (cachedUser) {
      console.log('✅ Auth cache HIT:', cachedUser.email || cachedUser.phone);
      req.user = cachedUser;
      return next();
    }
    ...
    const userData = {
      id: decoded.id,
      supabaseAuthId: decoded.supabaseAuthId,  // ✅ Changed from clerkUid
      phone: decoded.phone,                     // ✅ Added
      email: decoded.email,
      firstName: decoded.firstName,
      lastName: decoded.lastName,
